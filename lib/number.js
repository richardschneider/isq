...
function format(number, options) {
    ...
    if (options.minDigitsForSeparation <= digitTally(parts[0]))
        parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
    if (parts.length > 1 && options.minDigitsForSeparation <= digitTally(parts[1]))
