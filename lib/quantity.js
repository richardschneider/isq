...
Quantity.prototype.toString = function(options) {
    ...
    if (options.bestSymbol) {
        let system = Quantity.systems[options.system];
        if (!system)
            throw new Error("'" + options.system + "' is an undefined measurement system");

        let best = Object.keys(system.units)
            .map(key => system.units[key])
        ...
        if (best)
            return this.to(best.symbol).toString(options) + ' ' + best.symbol;
    }

    // Show the quantity in SI base units.
    let units = '';
    ...
    return (this.number.toString(options) + ' ' + units).trim();
};
...
/**
 * Converts the quanitity into another measurement system.
 *
 * @param {string} system - The name of a measurement system.
 * @param {object} [options]
 * @returns {string} - string representation of the quantity using symbols from the system.
 */
Quantity.prototype.in = function(system, options) {
...
units.add = function addSymbol(def, options) {
    ...
    if (!def.system)
        throw new Error('Missing measurement system for ' + (def.name || def.symbol));
    let system = systems[def.system];
    if (!system)
        system = systems[def.system] = { name: def.system, units: {} };
    ...
    system.units[def.name] = system.units[def.symbol] = Q;
...
units.add({ system: 'SI', name: 'gram', symbol: 'g', factor: 0.001, base: 'kg' }, {dontOverride: true});
